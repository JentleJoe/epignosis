...
const Experience = () => {
  ...
  return (
    <section id="experience" className="py-[100px] px-5 bg-gradient-to-br from-dark-surface to-dark-bg relative max-lg:py-20 max-md:py-[60px]">
      {/* Header */}
      ...
      <div className="max-w-[1200px] mx-auto grid grid-cols-2 gap-[60px] items-start max-lg:grid-cols-1">
        ...
        <div>
          <div className="flex flex-col">
            {experiences.map((exp, index) => (
              <AnimatedSection key={exp.id} animation="fadeRight" delay={index * 150} duration={700}>
                <div 
                  className="border-b border-[#333333] py-10 transition-all duration-300 hover:bg-[rgba(255,215,0,0.03)] hover:pl-5 hover:pr-5 hover:-ml-5 hover:-mr-5 max-md:py-[30px] group cursor-default"
                >
                  <div className="flex justify-between items-start gap-5 mb-2 max-md:flex-col max-md:gap-[10px]">
                    <h3 className="text-2xl font-bold text-white m-0 max-lg:text-[1.3rem] max-md:text-[1.2rem] group-hover:text-primary transition-colors duration-300">{exp.title}</h3>
                    <span className="bg-[rgba(255,215,0,0.15)] text-primary py-1.5 px-3.5 rounded font-bold text-[0.85rem] whitespace-nowrap tracking-[0.03em] group-hover:bg-primary group-hover:text-black transition-all duration-300">{exp.period}</span>
                  </div>
                  <p className="text-[#a0a0a0] text-[0.95rem] m-0 mb-[15px] font-medium">At {exp.company}</p>
                  <p className="text-[#808080] text-[0.95rem] leading-[1.7] m-0 group-hover:text-[#a0a0a0] transition-colors duration-300">{exp.description}</p>
                </div>
              </AnimatedSection>
            ))}
          </div>
        ...
        <AnimatedSection animation="fadeLeft" delay={200} duration={700}>
          <div>
            <div className="bg-[rgba(20,20,20,0.5)] border border-[#333333] rounded-xl py-10 px-[30px] transition-all duration-500 hover:border-primary/30 hover:shadow-[0_0_30px_rgba(255,215,0,0.05)]">
